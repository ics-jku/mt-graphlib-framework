...
void pinMode(pin_size_t pinNumber, PinMode pinMode) {
    ...
    gpio_init(digitalPinToPort(pinNumber), mode, VARIANT_GPIO_OSPEED,
              digitalPinToBitMask(pinNumber));
...
PinStatus digitalRead(pin_size_t pinNumber) {
    ...
    if ((uint32_t)LOW != (GPIO_ISTAT(digitalPinToPort(pinNumber)) &
                          digitalPinToBitMask(pinNumber))) {
        return HIGH;
    } else {
        return LOW;
...
void digitalToggle(pin_size_t pinNumber) {
    return gpio_bit_write(
        digitalPinToPort(pinNumber), digitalPinToBitMask(pinNumber),
        (bit_status)!gpio_output_bit_get(digitalPinToPort(pinNumber),
                                         digitalPinToBitMask(pinNumber)));
}
